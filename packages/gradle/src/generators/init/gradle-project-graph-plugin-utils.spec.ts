...
describe('Gradle Project Graph Plugin Utils', () => {
  describe('extractNxPluginVersion', () => {
    it.each([
      ...
      ['id(\'dev.nx.gradle.project-graph\') version("0.1.0")', '0.1.0'],
      ["id('dev.nx.gradle.project-graph') version('0.1.0')", '0.1.0'],
    ])('should extract version from %s', (input, expected) => {
      expect(extractNxPluginVersion(input)).toBe(expected);
    });

    it('should return null when no version is found', () => {
      expect(extractNxPluginVersion('some other content')).toBeNull();
    });
  });
